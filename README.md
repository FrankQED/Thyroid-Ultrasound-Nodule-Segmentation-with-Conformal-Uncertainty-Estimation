...
We adopt DeepLabV3+~\cite{deeplab} with an EfficientNet-B5 backbone pre-trained on ImageNet, and we explore the performance of three channel-attention modules: Squeeze-and-Excitation (SE)~\cite{se}, Convolutional Block Attention Module (CBAM) and Grouped Channel Attention (GCA); combined with two loss functions.
